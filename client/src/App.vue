    ...
    <main>
      <v-container class="mt-4">
        ...
        <v-snackbar v-model="authSnackbar" :timeout="5000" color=success bottom left>
                 ...
                 <h3>Agora você foi logado!</h3>
                 <v-btn dark flat @click="authSnackbar = false">Fechar</v-btn>
        </v-snackbar>
        <!-- AuthError snackbar -->
        <v-snackbar v-if="authError" v-model="authErrorSnackbar" :timeout="5000" color=info bottom left>
                 <v-icon class="mr-3">cancel</v-icon>
                 <h3>{{ authError.message }}</h3>
                 <v-btn dark flat to="/signin">Entrar</v-btn>
        </v-snackbar>
...
export default {
  ...
  watch: {
       ...
       authError(value) {
         // if authErro is not null, throw authError snackbar
         if(value !== null){
